...
1. **Recovery** - After crash/context loss, AI reads plan and resumes work
2. **Scoping** - Break work into chunks that fit in context window
3. **Dependencies** - Track what must be done first

## Usage

- **Starting work**: Create plan in `active/`
- **Resuming work**: Read plan, continue from current state
- **Finishing work**: Move to `completed/` or delete

Completed plans can be purged periodically - permanent decisions belong in AGENTS.md, code comments, or other documentation.
...
## Prerequisites
- [ ] other-plan-name (if dependent on another plan)
...
- Keep plans focused - one coherent unit of work
